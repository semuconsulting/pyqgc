...
1. The pyqgc version (`>>> pyqgc.version`)
...
4. A binary / hexadecimal dump of the QGC data stream.

**To Reproduce**

Steps to reproduce the behaviour:
1. Any relevant device configuration (if other than factory defaults).
2. Any causal QGC command input(s).
...
This information is typically output by the device at startup via a series of NMEA TXT messages. It can also be found by polling the device with a QGC MON-VER message. If you're using the PyGPSClient GUI, a screenshot of the QGCConfig window should suffice.
